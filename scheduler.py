"""
Greedy scheduler for PS1.

DESIGN ASSUMPTIONS (spec is ambiguous on these -- state them in your write-up):

  1. GRANULARITY: SCHEDULE_OCCUPANCY has no per-night field within a week
     (only activity_id, week, location_id, co_share_group). We therefore
     schedule at WEEKLY granularity: each activity uses AT MOST ONE access
     (one night's worth of work) per week, spread across as many weeks as
     needed to reach total_accesses. This is simpler and, importantly,
     SAFE: it can only under-use capacity relative to a true nightly model,
     never over-use it -- so it never risks a hard capacity violation, at
     the cost of possibly more overrun than a tighter nightly scheduler
     could achieve.

  2. CAPACITY: supply_capacity is treated as the max number of DISTINCT
     ACTIVITIES that may occupy a location in a given week (matching the
     legal-mix ceiling described in the spec: 1 PM alone / 1 PC+<=3C / <=4C
     -- these numbers match LOCATION_SUPPLY's capacity values exactly for
     normal locations (4) and interchange locations (1), which supports
     this reading).

  3. CO-SHARING: two activities can co-share a location-week only if they
     have IDENTICAL full footprints (this instance's activities rarely
     overlap partially, so this is a reasonable simplification). Legal mix
     (<=1 PM alone, <=1 PC + <=3C, or <=4C) is enforced among co-sharers.

  4. BUFFERS: buffer sectors extend the TUNNEL sector range only (not
     platforms). A buffer reservation blocks other (non-co-sharing)
     Live/Non-live(Consist) activities from using that location as either
     a closure or a buffer that same week; Non-live(Others) carries no
     buffer and isn't blocked by others' buffers (closures still apply).

  5. ORDERING: activities are scheduled in two passes -- those without a
     predecessor first (by contract_priority, then activity_priority, then
     planned_start_date), then those with a predecessor (same secondary
     sort), each starting no earlier than the week after its predecessor's
     last scheduled access.

Scenario A specifics: capacity is a hard 0-tolerance ceiling (never
exceeded), ECLO is forbidden (always 0), and activities are pushed to
later weeks (overrun) when blocked, never dropped.

SCENARIO B (updated per mentor guidance): "Strict Schedule, Flexible
Supply" is now read as broadly as possible -- EVERY location-based
packing/spacing rule (raw numeric capacity, buffer safety spacing, and
PC/PM turn-taking exclusivity) is treated as flexible "supply" that can
be exceeded, tracked, and scored via excess_access_nights, rather than as
a hard rule. Only workload conservation, weekly-allocation + workfront
caps, and predecessor precedence remain genuinely hard for B -- and,
together with the hard deadline, these are proven satisfiable
simultaneously (verified via CP-SAT: OPTIMAL status), which is what
guarantees zero overrun is actually achievable, matching the spec's own
claim that "a feasible Scenario B submission has zero overrun by
construction."
"""
